...
def proc_patmax(inputs, params):
    """
    CogPatMaxPatternAlignTool — Pattern matching nâng cao.
    Tìm kiếm template đã train, xuất vị trí, góc, score.
    Template được vẽ trong NodeDetailDialog và lưu vào params["_template_array"].
    """
    img = inputs.get("image")
    if img is None:
        return {"image":None,"found":False,"score":0.0,"x":0.0,"y":0.0,
                "angle":0.0,"scale":1.0,"num_found":0}

    templ = params.get("_template_array", None)
    if templ is None:
        vis = _bgr(img.copy())
        cv2.putText(vis,"[PatMax] No template trained",(10,30),
                    cv2.FONT_HERSHEY_SIMPLEX,0.7,(0,150,255),2)
        cv2.putText(vis,"Double-click → draw ROI on image",(10,55),
                    cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,100,180),1)
        return {"image":vis,"found":False,"score":0.0,"x":0.0,"y":0.0,
                "angle":0.0,"scale":1.0,"num_found":0}

    gi = _gray(img); gt = _gray(templ)
    if gt.shape[0]>=gi.shape[0] or gt.shape[1]>=gi.shape[1]:
        return {"image":img,"found":False,"score":0.0,"x":0.0,"y":0.0,
                "angle":0.0,"scale":1.0,"num_found":0}

    # Multi-angle search
    method = cv2.TM_CCOEFF_NORMED
    best_score = -1; best_loc = (0,0); best_angle = 0.0
    angle_range = params.get("angle_range", 0)
    angle_step  = max(1, params.get("angle_step", 5))

    angles = [0.0]
    if angle_range > 0:
        angles = list(np.arange(-angle_range, angle_range+angle_step, angle_step))

    h_t, w_t = gt.shape[:2]
    for ang in angles:
        if ang != 0:
            M = cv2.getRotationMatrix2D((w_t//2,h_t//2), ang, 1.0)
            gt_rot = cv2.warpAffine(gt, M, (w_t,h_t))
        else:
            gt_rot = gt
        if gt_rot.shape[0]>=gi.shape[0] or gt_rot.shape[1]>=gi.shape[1]:
            continue
        res = cv2.matchTemplate(gi, gt_rot, method)
        _, mv, _, ml = cv2.minMaxLoc(res)
        if mv > best_score:
            best_score = mv; best_loc = ml; best_angle = ang

    threshold = params.get("accept_threshold", 0.5)
    found = best_score >= threshold
    tx, ty = best_loc
    cx = float(tx + w_t/2); cy = float(ty + h_t/2)

    vis = _bgr(img.copy())
    color = (0,220,80) if found else (0,80,255)
    if found:
        # Draw found pattern box
        pts = np.array([[tx,ty],[tx+w_t,ty],[tx+w_t,ty+h_t],[tx,ty+h_t]], np.float32)
        if best_angle != 0:
            ctr = np.array([cx,cy])
            M2  = cv2.getRotationMatrix2D((cx,cy), -best_angle, 1.0)
            pts_h = np.hstack([pts, np.ones((4,1))])
            pts   = (M2 @ pts_h.T).T
        pts = pts.astype(np.int32)
        cv2.polylines(vis, [pts], True, color, 2)
        # Crosshair at center
        cv2.line(vis,(int(cx)-12,int(cy)),(int(cx)+12,int(cy)),color,2)
        cv2.line(vis,(int(cx),int(cy)-12),(int(cx),int(cy)+12),color,2)
        cv2.circle(vis,(int(cx),int(cy)),4,color,-1)
        cv2.putText(vis,f"Score:{best_score:.3f} Ang:{best_angle:.1f}°",
                    (tx,ty-8),cv2.FONT_HERSHEY_SIMPLEX,0.55,color,2)
    else:
        cv2.putText(vis,f"PatMax: NOT FOUND (best={best_score:.3f})",
                    (10,30),cv2.FONT_HERSHEY_SIMPLEX,0.65,(0,60,255),2)

    num = params.get("num_results",1)
    return {"image":vis,"found":found,"score":float(best_score),
            "x":cx,"y":cy,"angle":float(best_angle),"scale":1.0,
            "num_found":1 if found else 0}

def proc_patfind(inputs, params):
    """CogPMAlignTool — Pattern Find (normalized cross-correlation, nhanh hơn PatMax)."""
    img = inputs.get("image")
    if img is None:
        return {"image":None,"found":False,"score":0.0,"x":0.0,"y":0.0,"num_found":0}
    templ = params.get("_template_array", None)
    if templ is None:
        vis = _bgr(img.copy())
        cv2.putText(vis,"[PatFind] No template",(10,30),cv2.FONT_HERSHEY_SIMPLEX,0.7,(0,150,255),2)
        return {"image":vis,"found":False,"score":0.0,"x":0.0,"y":0.0,"num_found":0}

    gi = _gray(img); gt = _gray(templ)
    if gt.shape[0]>=gi.shape[0] or gt.shape[1]>=gi.shape[1]:
        return {"image":img,"found":False,"score":0.0,"x":0.0,"y":0.0,"num_found":0}

    res = cv2.matchTemplate(gi, gt, cv2.TM_CCOEFF_NORMED)
    _, mv, _, ml = cv2.minMaxLoc(res)
    h_t,w_t = gt.shape[:2]
    tx,ty = ml; cx=float(tx+w_t/2); cy=float(ty+h_t/2)
    thresh = params.get("accept_threshold",0.5)
    found  = mv >= thresh
    vis    = _bgr(img.copy())
    col    = (0,220,80) if found else (0,80,255)
    if found:
        cv2.rectangle(vis,(tx,ty),(tx+w_t,ty+h_t),col,2)
        cv2.line(vis,(int(cx)-10,int(cy)),(int(cx)+10,int(cy)),col,2)
        cv2.line(vis,(int(cx),int(cy)-10),(int(cx),int(cy)+10),col,2)
        cv2.putText(vis,f"{mv:.3f}",(tx,ty-6),cv2.FONT_HERSHEY_SIMPLEX,0.55,col,2)
    else:
        cv2.putText(vis,f"PatFind: NOT FOUND ({mv:.3f})",
                    (10,30),cv2.FONT_HERSHEY_SIMPLEX,0.65,(0,60,255),2)
    return {"image":vis,"found":found,"score":float(mv),"x":cx,"y":cy,"num_found":1 if found else 0}
...
def proc_crop(inputs, params):
    img=inputs.get("image")
    if img is None: return {"image":None,"roi_image":None,"x":0,"y":0,"w":0,"h":0}
    x=int(inputs.get("x",params.get("x",0))); y=int(inputs.get("y",params.get("y",0)))
    cw=int(inputs.get("w",params.get("crop_w",320))); ch=int(inputs.get("h",params.get("crop_h",240)))
    ih,iw=img.shape[:2]
    x=max(0,min(x,iw-1)); y=max(0,min(y,ih-1))
    cw=max(1,min(cw,iw-x)); ch=max(1,min(ch,ih-y))
    roi=img[y:y+ch,x:x+cw]
    vis=_bgr(img.copy())
    cv2.rectangle(vis,(x,y),(x+cw,y+ch),(0,212,255),2)
    cv2.putText(vis,f"ROI({x},{y}) {cw}×{ch}",(x,max(0,y-6)),cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,212,255),1)
...
CATEGORIES = [
    ...
    "Calibration","Logic & Flow","Output & Display"
]
