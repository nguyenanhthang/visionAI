...
def resolve_obj_origin_xy(r, obj_idx: int, overrides=None):
    """Return (x, y) image-coord cho marker origin của object detect thứ obj_idx.

    Quy tắc:
      - Override per-object (nếu có trong dict overrides[obj_idx]) → thắng.
      - Obj 0 → (r.origin_x, r.origin_y) — pattern-origin transformed
        (giá trị custom mà user đã set qua Pattern Origin Point khi train).
      - Obj 1+ → (r.x, r.y) — tâm bounding box của match đó (default
        không kế thừa pattern-origin offset).
    """
    if overrides:
        ov = overrides.get(obj_idx) if isinstance(overrides, dict) else None
        if ov is not None:
            try:
                ...
                pass
    if obj_idx == 0:
        return float(r.origin_x), float(r.origin_y)
...
def draw_patmax_results(image: np.ndarray,
    ...
    # Pattern dimensions cho ref transform (refs d\u00f9ng pattern-local coords).
    pw = ph = 0.0
    ...
    # Khi >1 object: prefix label "Obj N: " \u0111\u1ec3 ph\u00e2n bi\u1ec7t object n\u00e0o s\u1edf h\u1eefu
    # marker / ref. \u0110\u01a1n object \u2192 gi\u1eef label g\u1ecdn kh\u00f4ng prefix.
    n_found = sum(1 for rr in results if rr.found)
    multi_obj = n_found > 1
    ...
    for i, r in enumerate(results):
        ...
        # T\u00ednh trigonometry (c\u1ea7n cho c\u1ea3 origin v\u00e0 ref transforms)
        rad_o = math.radians(-r.angle)
        ca = math.cos(rad_o); sa = math.sin(rad_o)
        sc = r.scale if r.scale else 1.0

        # Resolve origin marker position theo per-object rule:
        # obj 0 \u2192 pattern-origin transformed (nh\u01b0 c\u0169); obj 1+ \u2192 t\u00e2m v\u1eadt th\u1ec3.
        # Override (n\u1ebfu user k\u00e9o) th\u1eafng default.
        ...
        if show_xy:
            ...
            for ref in extra_refs:
                ...
                el_txt = f"{obj_prefix}{r_name} ({exf:.1f},{eyf:.1f})"
