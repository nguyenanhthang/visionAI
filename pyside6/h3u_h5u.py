...
def read_data_h3u(ip: str, address: int) -> int | None:
    try:
        with _lock:
            v = _get_master(ip).execute(
                SLAVE_ID, cst.READ_HOLDING_REGISTERS, int(address), 1,
            )
...
def read_multi_data_h3u(ip: str, start: int, count: int) -> list[int] | None:
    try:
        with _lock:
            v = _get_master(ip).execute(
                SLAVE_ID, cst.READ_HOLDING_REGISTERS, int(start), int(count),
            )
...
def write_data_h3u(ip: str, address: int, value: int) -> bool:
    try:
        with _lock:
            _get_master(ip).execute(
                SLAVE_ID, cst.WRITE_SINGLE_REGISTER, int(address),
