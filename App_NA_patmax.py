"""
App_0408.py — App AOI TESLA bản PatQuick (CogPatMax pattern match) của
VisionPro/core/patmax_engine, chuyển từ Faster R-CNN (torch).

Khác biệt vs bản torch cũ:
  ...
  • Option menu thêm "Train PatMax" — train lại pattern từ frame camera.
  • Logic OK/NG giữ nguyên: count == 3 + 3 moment trong [0.4, 0.6] → OK.

Cần: thư mục `VisionPro/` (chứa core/patmax_engine.py) ở cùng cấp file
script HOẶC trong PYTHONPATH. Model files (`model.json` + `model.npz`)
mặc định đặt cạnh script (override qua _PATMAX_MODEL_PATH).
"""
import os
import sys
...
from patmax_engine import load_model, run_patmax_align, PatMaxModel
...
#   _SEARCH_DOWNSCALE   : search ở 1/N resolution cho nhanh. 3 ≈ 9× nhanh
#                         vs full-res, sai số vị trí ±3px (đủ để đếm vít).
#                         Đo trên model thật: ds2=792ms → ds3+step15=411ms
#                         → ds4=315ms. Tăng nếu cần nhanh hơn nữa.
...
_SEARCH_DIAG = True
_SEARCH_DIAG_FLOOR = 0.72   # score tối thiểu để vẽ candidate diagnostic
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.lbl_name_staffid = customtkinter.CTkLabel(self.frame_top, text='Employee ID', font=customtkinter.CTkFont('Times new roman', 16, 'normal'))
        self.lbl_name_staffid.grid(row=1, column=0, padx=(100, 10), pady=0)

        self.btn_name_staffid = customtkinter.CTkButton(self.frame_top, text=self.staff_id, font=customtkinter.CTkFont('Times new roman', 16, 'normal'))
    ...
    def _train_patmax_interactive(self):
        """Train PatMax từ frame camera hiện tại:
          ...
          2. _select_ellipse_roi để user vẽ ellipse quanh 1 con vít
          3. Confirm → train_patmax + save_model (override model.json+.npz)
          4. Reload self.patmax_model — AOI() lần sau dùng model mới

        Search params (accept_threshold, num_results, overlap_threshold…)
        GIỮ từ model cũ nếu có — train mới không reset tuning của user."""
        from patmax_engine import train_patmax, save_model
        # 1. Capture frame
        # sleep(0.5)
        # frame = None
        # for _ in range(4):
        ...
        # 4. Train (mode "evaluate") — preserve search tuning từ model cũ
    ...
    def Lock_QC(self):
        ...
        try:
            ser_lock = serial.Serial(self.scan, 9600, 8, timeout=1)
            while True:
                if ser_lock.isOpen():
                    ser_lock.write(bytearray(b'\x01\x54\x04'))
                else:
                    ser_lock.open()
                    ser_lock.write(bytearray(b'\x01\x54\x04'))
                data = ser_lock.readline(100)
                data_scan = str(data)[2:-3]
                value_product_id = data_scan
                if len(value_product_id) > 0:
                    value_main = value_product_id
                    if value_main == lock:
    ...
    def AOI(self, moment1, moment2, moment3):
        ...
        # on_off_material = self.sys.find('Material')
        ...
        cv2.rectangle(self.img, (166, 195), (516, 400), (255, 255, 255), -1)
        ...
        # ── OK/NG verdict ──────────────────────────────────────────
        # PASS: tìm thấy đúng 3 screws + 3 moments trong range [0.4, 0.6].
        # PatMax model = 1 pattern duy nhất → mọi result đều là screw,
        # không cần check label như bản torch.
        ok = (count == 3
              and 0.4 <= moment1 <= 0.6
              and 0.4 <= moment2 <= 0.6
              and 0.4 <= moment3 <= 0.6)
        ...
        if self.check_sample is False:
            ...
            #     self.re_screws = self.re_screws - 3
            #     self.sys.replace(str(self.re_screws), 'remainning_screws')
            #     self.btn_remainning_screws.configure(text = str(self.re_screws))
