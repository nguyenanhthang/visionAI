...
class MainWindow(QMainWindow):
    ...
    def _run_worker(self, worker, busy_attr: str | None = None):
        ...
        TRẦN ``_MAX_JOBS``: nếu job nền đang dồn (share/mạng/Excel kẹt, hoặc
        PLC bắn verdict dồn dập) thì BỎ job mới thay vì đẻ thêm thread —
        đẻ thread vô hạn = cạn handle/bộ nhớ → đơ cứng máy (phải tắt Task
        Manager). Thà rớt 1 lượt đẩy/ghi còn hơn đơ cả app.
        """
    ...
    def _append_log(self, message: str, tag: str, level: str, detail: str):
        ...
        crash.log (Timeout 11/06) bắt được GUI đơ cứng ≥15s NGAY TRONG
        ``log_view.append``: mỗi lần widget đổi nội dung, Windows bắn thông
        báo accessibility đồng bộ sang tool đang attach (remote/AV) — tool
        treo là app treo theo. Đệm + flush 1 lần mỗi 200ms: (1) giảm hàng
        chục lần số lần gọi widget, (2) log dồn dập (bão lỗi) không còn
        nghẽn GUI. Kết hợp chặn WM_GETOBJECT ở main.py để miễn nhiễm hẳn.
        """
        tag_colors = {
            "OK":  "#7ee787",
            "NG":  "#ffa198",
