...
class ImageViewerPanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        view_lbl = QLabel(tr("👁  IMAGE VIEWER"))
        ...
        self._btn_multi_mode.setText(tr("Auto ▾"))
        ...
        self._btn_multi_mode.setToolTip(
            tr("Chọn layout mode cho multi-view:\n"
        ...
        self._act_mode_auto   = _mm.addAction(tr("Auto — 1 ô per Acquire/Camera"))
        self._act_mode_custom = _mm.addAction(tr("Custom — tự thêm view (right-click)"))
        ...
        self._lbl_size   = QLabel(tr("—"))
        self._lbl_pixel  = QLabel(tr("Hover over image for pixel info"))
        self._lbl_status = QLabel(tr("IDLE"))
    ...
    def _rebuild_multi_grid(self):
        ...
        for i, (root_id, default_nid) in enumerate(cells_plan):
            ...
            """)
            cb.setToolTip(
                tr("Chọn base node hiển thị trong ô — list gom cả Acquire Image "
            ...
            if self._multi_mode == "custom":
                ...
                btn_x.setToolTip(tr("Remove view này"))
                btn_x.setStyleSheet(
    ...
    def _on_multi_canvas_menu(self, sender, pos):
        ...
        if self._multi_mode == "custom":
            if len(self._custom_views) >= 9:
                a = menu.addAction(tr("Đã đạt cap 9 view"))
                a.setEnabled(False)
            else:
                a = menu.addAction(tr("＋  Add View"))
                a.triggered.connect(self._add_custom_view)
        else:
            a = menu.addAction(tr("Chuyển sang Custom mode để thêm view"))
    ...
    def _on_node_selected(self, idx: int):
        ...
        if node_id:
            ...
            self._lbl_size.setText(tr("—"))
            self._lbl_status.setText(tr("IDLE"))
    ...
    def _rebuild_results_menu(self):
        ...
        if total_tools == 0:
            wa = QWidgetAction(menu)
            lbl = QLabel(tr("  (Chưa có tool nào trong pipeline)  "))
            lbl.setStyleSheet("color:#64748b; padding:8px;")
            wa.setDefaultWidget(lbl)
            menu.addAction(wa)
        else:
        ...
        # Quick actions
        from PySide6.QtGui import QAction
        act_all = QAction(tr("✓  Select All"), menu)
        act_none = QAction(tr("✗  Clear All"), menu)
    ...
    def _overlay_diff(self, base: np.ndarray, before: np.ndarray,
        ...
        Delegate sang core.overlay_render để Image Viewer và Save Image tool
        dùng CHUNG 1 thuật toán → ảnh lưu giống hệt ảnh xem."""
        from core.overlay_render import overlay_diff
    ...
    def _display_node(self, node_id: str):
        ...
        if img is not None and isinstance(img, np.ndarray):
            ...
            self._img_view.set_image(None)
            self._lbl_size.setText(tr("No image output yet"))
