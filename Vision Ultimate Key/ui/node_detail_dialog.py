...
from ui.properties_panel import ParamRow, OverlaySelectRow, CsvColumnsRow
...
class InteractiveImageLabel(QLabel):
    ...
    def _render(self):
        # Bọc try/except: _render được gọi từ slot (resizeEvent / set_image sau
        # Auto Run). Trên PySide6, exception chưa bắt trong slot làm THOÁT app.
        # Lỗi vẽ thoáng qua lúc Auto Run cập nhật liên tục → log & bỏ qua thay
        # vì crash.
        try:
            self._render_impl()
        except Exception as e:
            import traceback
            print(f"[NodeDetail._render] {e}")
            traceback.print_exc()
    ...
    def _render_impl(self):
        ...
        if self._scroll_area is not None and self._user_zoom > 1.0:
            ...
            # resize() có thể gọi resizeEvent ĐỒNG BỘ (Windows) → _render tái
            # nhập giữa chừng. Cờ _applying_resize chặn vòng lặp đó.
            self._applying_resize = True
            try:
                if self.width() != need_w or self.height() != need_h:
                    self.setMinimumSize(need_w, need_h)
                    self.resize(need_w, need_h)
            finally:
                self._applying_resize = False
    ...
    def resizeEvent(self, event):
        # Bỏ qua _render khi đang ở trong resize() do chính _render gọi → tránh
        # đệ quy resize↔render (crash/giật khi Auto Run cập nhật ảnh liên tục).
        if not getattr(self, "_applying_resize", False):
            self._render()
...
# Giữ tham chiếu WORKER tương tự. Worker là QObject KHÔNG parent, đã moveToThread;
# khi self._run_worker bị ghi đè bởi lần Auto Run kế, nếu không có ref sống khác
# thì Python GC xoá C++ object trong lúc thread chưa finished → SEGFAULT.
_RUN_WORKERS: set = set()
...
class _NodeRunWorker(QObject):
    ...
    def __init__(self, node, inputs, params=None):
        super().__init__()
        self._node = node
        self._inputs = inputs
        # Snapshot params: kéo slider lúc node đang chạy KHÔNG được sửa dict mà
        # worker đang đọc ở thread khác (race → crash / kết quả lẫn lộn).
        self._params = params if params is not None else dict(node.params)

    def run(self):
        try:
            out = self._node.tool.process_fn(self._inputs, self._params) or {}
            status = "pass"
            if "pass" in out:
...
class NodeDetailDialog(QDialog):
    ...
    def __init__(self, node: NodeInstance, graph: FlowGraph, parent=None):
        ...
        self._auto_run_cb = QCheckBox(tr("Auto Run"))
        ...
        self._auto_run_cb.setToolTip(tr("Tự động chạy node khi thay đổi tham số"))
        ...
        self._run_btn = QPushButton(tr("▶  Run Node"))
        ...
        spl.setHandleWidth(6)                       # đủ rộng để kéo dãn dễ
        spl.setChildrenCollapsible(False)           # không thu panel về 0
        spl.setStyleSheet(
            "QSplitter::handle{background:#1e2d45;}"
            "QSplitter::handle:hover{background:#00d4ff;}")
        root.addWidget(spl, 1)

        # LEFT — params + output values. KHÔNG cap maxWidth → kéo splitter rộng
        # tuỳ ý để xem hết nội dung (mask/giá trị dài). Chỉ giữ min cho gọn.
        left = QWidget(); left.setMinimumWidth(240)
        ...
        tabs.addTab(self._params_scroll, tr("⚙ Params"))
        tabs.addTab(self._build_ports_widget(), tr("🔌 Ports"))
        ll.addWidget(tabs, 3)            # params chiếm phần lớn, Output Values cuộn riêng

        self._out_group = QGroupBox(tr("Output Values"))
        og = QVBoxLayout(self._out_group)
        og.setContentsMargins(8, 12, 8, 8); og.setSpacing(4)
        self._out_labels = {}
        self._build_output_labels(og)
        # Bọc trong scroll → nhiều output / giá trị dài (mask, list…) vẫn xem
        # hết bằng cách cuộn; không bị cắt khi cửa sổ thấp.
        out_scroll = QScrollArea()
        out_scroll.setWidgetResizable(True)
        out_scroll.setFrameShape(QFrame.NoFrame)
        out_scroll.setWidget(self._out_group)
        out_scroll.setMinimumHeight(120)
        ll.addWidget(out_scroll, 2)

        self._status_lbl = QLabel(tr("Status: IDLE"))
        ...
        # Tools không xuất ảnh (Logic & Flow / Output: math, compare, judge,
        # logic_*, save_image, csv_log, yield_stats, line_relation…) → bỏ hẳn
        # panel OUTPUT IMAGE (luôn "No Image"), chỉ hiển thị Params + Output
        # Values. Dummy image attrs để refresh_outputs dùng chung không cần
        # if/else khắp nơi (giống Script Tool ở trên).
        tool_has_image_output = any(p.data_type == "image" for p in tool.outputs)
        if not tool_has_image_output:
            self._img_label = None
            self._img_info  = QLabel("")
            self._hover_bar = QLabel("")
            self._pixel_bar = QLabel("")
            right.deleteLater()          # right pane không dùng tới
            # Cho Params + Output Values giãn full chiều rộng (bỏ cap 300px
            # vốn dành chỗ cho ảnh) + thu nhỏ cửa sổ vì không còn vùng ảnh.
        ...
        img_hdr = QHBoxLayout()
        img_lbl = QLabel(tr("OUTPUT IMAGE"))
        ...
        if tool.tool_id in ("color_segment", "color_picker", "color_match"):
            ...
            if tool.tool_id == "color_segment":
                self._pick_btn = QPushButton(tr("🎯 Pick Color"))
                self._pick_btn.setCheckable(True)
                self._pick_btn.setFixedHeight(26)
                self._pick_btn.setToolTip(
                    tr("Click rồi click vào ảnh để lấy màu pixel làm màu lọc. "
        ...
        if tool.tool_id == "crop_roi":
            ...
            if self._roi_port_connected:
                self._mode_hint.setText(
                    tr("🔗  Port x/y/w/h kết nối — ROI auto-tracking. Có thể "
                    "kéo override nhưng Run kế tiếp sẽ snap về port value."))
            else:
                self._mode_hint.setText(
                    tr("✏  Kéo vẽ ROI; sau khi vẽ kéo body để di chuyển hoặc "
                    "4 góc để resize."))
            ...
            self._mode_hint.setText(
                tr("🎯  PatMax/PatFind — Cửa sổ Train & Search chuyên dụng đang mở..."))
            self._mode_hint.setStyleSheet(
            ...
            self._mode_hint.setText(
                tr("🎨  Click chuột vào ảnh để lấy màu tại điểm đó. "
        ...
        spl.addWidget(right)
        # Default rộng hơn cho panel trái (xem Output Values thoải mái); user
        # vẫn kéo splitter chỉnh tuỳ ý vì đã bỏ cap maxWidth.
        spl.setSizes([360, 760])
    ...
    def _build_script_editor_panel(self, layout: QVBoxLayout):
        ...
        hint = QLabel(
            tr("<span style='color:#94a3b8'>Inputs:</span> "
    ...
    def _build_params_widget(self) -> QWidget:
        ...
        # (pipeline cũ / tool thêm param mới / file lỗi) → color_space luôn hợp lệ;
        # visible_if không ẩn nhầm toàn bộ slider ngưỡng (combo vẫn hiện "HSV"
        # nhưng key vắng/sai ⇒ get()≠"HSV" ⇒ ẩn hết).
        ...
        if tool.tool_id == "yolo_detect":
            self._yolo_info_lbl = QLabel(tr("📂  Add file .pt hoặc .onnx để bắt đầu"))
        ...
        lay.addWidget(sep)
        note = QLabel(tr("▶ Run Node để áp dụng thay đổi"))
    ...
    def _refresh_yolo_info(self):
        ...
        if not path:
            lbl.setText(tr("📂  Add file .pt hoặc .onnx để bắt đầu"))
            lbl.setStyleSheet(
                "color:#94a3b8; font-size:11px; background:#0a0e1a;"
                "border:1px solid #1e2d45; border-radius:4px;"
                "padding:8px; font-family:'Courier New';")
    ...
    def _on_reset_crop_image(self):
        ...
        if src_img is None:
            QMessageBox.information(self, tr("Reset Image"),
                tr("Chưa có ảnh nguồn — chạy pipeline trước rồi reset."))
            return
        h, w = src_img.shape[:2]
    ...
    def _open_patmax_dialog(self):
        ...
        dlg.show()
        # Update mode hint
        self._mode_hint.setText(tr("🎯  PatMax dialog mở — Train & Search tại đó."))
    ...
    def _on_pixel_hovered(self, x: int, y: int):
        """Rê chuột trên ảnh → cập nhật hover bar màu. GIỮ giá trị pixel cuối
        khi con trỏ ra ngoài vùng ảnh (không ẩn) để thanh màu luôn hiển thị."""
        bgr = self._sample_color_bgr(x, y)
        if bgr is None:
            return   # ngoài ảnh → giữ nguyên giá trị cuối, không ẩn
        self._hover_bar.setText(self._format_color_text(x, y, bgr))
        self._hover_bar.show()

    def _on_image_mouse_left(self):
        # Ghim thanh màu: giữ nguyên giá trị pixel cuối cùng thay vì ẩn đi khi
        # con trỏ rời khỏi ảnh (theo yêu cầu — thanh không biến mất).
        pass
    ...
    def _on_run(self):
        ...
        if getattr(self, "_run_btn", None):
            ...
            self._run_btn.setText(tr("⏳  Đang chạy…"))

        # Chạy process_fn ở thread nền → không treo UI (OCR easyocr có thể vài giây).
        # Snapshot params NGAY tại đây → kéo slider sau đó không sửa dict worker đọc.
        thread = QThread()
        worker = _NodeRunWorker(node, inputs, dict(node.params))
        ...
        thread.finished.connect(lambda w=worker: _RUN_WORKERS.discard(w))
        self._run_thread = thread
        self._run_worker = worker
        _RUN_THREADS.add(thread)
        _RUN_WORKERS.add(worker)         # giữ sống tới khi thread finished
        thread.start()
    ...
    def _on_run_done(self, outputs: dict, status: str, error: str):
        ...
        if getattr(self, "_run_btn", None):
            ...
            self._run_btn.setText(tr("▶  Run Node"))
        # Refresh UI bọc try/except: exception chưa bắt trong slot làm THOÁT app
        # trên PySide6. Auto Run cập nhật dồn dập → lỗi thoáng qua không giết app.
        try:
            self.refresh_outputs()
            self.run_requested.emit(node.node_id)
        except Exception as e:
            import traceback
            print(f"[NodeDetail._on_run_done] {e}")
            traceback.print_exc()
    ...
    def refresh_outputs(self):
        ...
        for name, lbl in self._out_labels.items():
            ...
            if val is None:
                lbl.setText(tr("—")); lbl.setStyleSheet("color:#1e2d45; font-size:11px;")
