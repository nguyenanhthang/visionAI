...
class PatMaxDialog(QDialog):
    ...
    def __init__(self, node: NodeInstance, graph: FlowGraph, parent=None):
        ...
        logo = QLabel(tr("🎯  TPatMaxPatternAlignTool"))
        logo.setStyleSheet(
        ...
        # Status chip
        self._status_chip = QLabel(tr("● UNTRAINED"))
        ...
        # Model preview
        ll.addWidget(QLabel(tr("MODEL STATUS")))
        ...
        self._tabs.addTab(self._build_train_tab(), tr("⚙ Train"))
        self._tabs.addTab(self._build_search_tab(), tr("▶ Search"))
        ...
        # Result table
        ll.addWidget(QLabel(tr("RESULTS")))
        ...
        self._btn_refresh.setToolTip(
            tr("Lấy ảnh mới từ upstream — xoá result_vis/score_map cũ, "
            "giữ ROI + origin"))
        self._btn_refresh.clicked.connect(self._on_refresh_click)
        itl.addWidget(self._btn_refresh)

        # Zoom buttons — style giống NodeDetailDialog.
        itl.addSpacing(12)
        def _zb(txt, tip):
        ...
        # Wire zoom buttons → label methods (label đã có set_scroll_area).
        ...
        # Status bar
        self._img_status = QLabel(tr("Load image source → Run pipeline → Draw ROI → Train"))
    ...
    def _build_train_tab(self) -> QWidget:
        ...
        hint = QLabel(tr("1. Chọn loại shape ROI (Rect/Circle/Ellipse/Polygon)\n"
        ...
        self._roi_mode_combo.setToolTip(
            tr("Single: 1 ROI duy nhất (vẽ lại = thay).\n"
        ...
        self._shape_list_lbl = QLabel(tr("Shapes: 0"))
        ...
        btn_clear_shapes = QPushButton(tr("🗑  Clear All Shapes"))
        ...
        # Shape selector
        shape_grp = QGroupBox(tr("ROI Shape"))
        ...
        orig_grp = QGroupBox(tr("Pattern Origin Point"))
        ...
        lbl_x = QLabel(tr("X:")); lbl_x.setStyleSheet("color:#94a3b8; font-size:11px;")
        ...
        lbl_y = QLabel(tr("Y:")); lbl_y.setStyleSheet("color:#94a3b8; font-size:11px;")
        ...
        # Hint
        hint_o = QLabel(tr("💡 Kéo dấu O vàng — có thể kéo ra ngoài ROI"))
        hint_o.setStyleSheet("color:#ffd700; font-size:10px;")
        hint_o.setWordWrap(True)
        og.addWidget(hint_o)

        # Reset Image button
        btn_reset = QPushButton(tr("🔄  Reset Image"))
        ...
        # Canny params
        canny_grp = QGroupBox(tr("Edge Detection (Canny)"))
        ...
        # Train Mode
        tm_grp = QGroupBox(tr("Train Mode"))
        self._tm_grp = tm_grp
        if is_align_tool:
        ...
        tm_hint = QLabel(
            tr("• Evaluate DOFs At Runtime: train nhanh, search xử lý DOF khi run\n"
        ...
        # Train button
        self._btn_train = QPushButton(tr("⚙  Train Pattern"))
    ...
    def _build_search_tab(self) -> QWidget:
        ...
        # Search button
        self._btn_search = QPushButton(tr("▶  Run Search"))
        ...
        self._search_summary = QLabel(tr("No search run yet."))
        ...
        self._btn_show_score = QPushButton(tr("🗺  View Score Map"))
    ...
    def _build_settings_tab(self) -> QWidget:
        ...
        # Threshold
        thr_grp = QGroupBox(tr("Acceptance Threshold"))
        ...
        # Angle
        ang_grp = QGroupBox(tr("Angle Search"))
        ag = QVBoxLayout(ang_grp); ag.setContentsMargins(8,12,8,8); ag.setSpacing(4)
        self._chk_angle = QCheckBox(tr("Enable angle search"))
        ...
        # Scale
        sc_grp = QGroupBox(tr("Scale Search"))
        sg = QVBoxLayout(sc_grp); sg.setContentsMargins(8,12,8,8); sg.setSpacing(4)
        self._chk_scale = QCheckBox(tr("Enable scale search"))
        ...
        # Multi-result
        mr_grp = QGroupBox(tr("Multi-Result"))
        ...
        # Search region
        sr_grp = QGroupBox(tr("Search Region (optional)"))
        srg = QVBoxLayout(sr_grp); srg.setContentsMargins(8,12,8,8); srg.setSpacing(4)
        hint_sr = QLabel(tr("0 = search entire image"))
        ...
        self._chk_show_xy = QCheckBox(tr("Show X, Y reference (origin + axes)"))
        ...
        self._chk_show_bbox = QCheckBox(tr("Show bounding box (+ score label)"))
        ...
        hint_disp = QLabel(
            tr("Bật/tắt từng phần overlay vẽ lên ảnh output pipeline."))
    ...
    def _on_refresh_click(self):
        ...
        if self._refresh_image_if_changed():
            ...
            self._img_status.setText(tr("⚠ Không có ảnh upstream để refresh."))
    ...
    def _refresh_image_if_changed(self) -> bool:
        ...
        if hasattr(self, "_search_summary"):
            self._search_summary.setText(tr("Image mới — chạy search lại."))
            self._search_summary.setStyleSheet(
                "background:#0d1220; color:#94a3b8; font-size:11px;"
                "font-family:'Courier New'; padding:8px; border-radius:4px;")
    ...
    def _on_clear_shapes(self):
        ...
        self._models = []
        if hasattr(self, "_shape_list_lbl"):
            self._shape_list_lbl.setText(tr("Shapes: 0"))
    ...
    def _reset_image(self):
        ...
        if img is not None and isinstance(img, np.ndarray):
            ...
            self._img_status.setText(tr("⚠ Không có ảnh upstream để reset."))
        # Clear search summary
        if hasattr(self, "_search_summary"):
            self._search_summary.setText(tr("Image reset — chạy search lại nếu cần."))
            self._search_summary.setStyleSheet(
                "background:#0d1220; color:#94a3b8; font-size:11px;"
                "font-family:'Courier New'; padding:8px; border-radius:4px;")
        if hasattr(self, "_result_table"):
    ...
    def _on_origin_dragged(self, ox: float, oy: float):
        """Kéo origin chính của pattern (image coords)."""
        self._origin_updating = True
        try:
            self._sp_origin_x.setValue(float(ox))
            self._sp_origin_y.setValue(float(oy))
    ...
    def _on_origin_angle_dragged(self, angle: float):
        """Origin chính không có angle field, bỏ qua."""
        return

    def _on_extra_origin_dragged(self, idx: int, ox: float, oy: float):
        """User kéo marker của extra ref idx trên canvas (image coords).
        Convert sang pattern-local và lưu vào model.extra_refs.
        """
        refs = self._extras()
        if idx < 0 or idx >= len(refs):
            return
        rx0, ry0 = self._roi_origin_xy()
        refs[idx]["x"] = float(ox) - rx0
        refs[idx]["y"] = float(oy) - ry0
        self._node.params["_patmax_model"] = self._model
        # Sync list label + spinbox nếu ref này đang được select
        self._refresh_ref_list_item(idx)
        if hasattr(self, "_ref_list") and self._ref_list.currentRow() == idx:
    ...
    def _on_extra_origin_angle_dragged(self, idx: int, angle: float):
        """User xoay marker của extra ref idx (handle vàng)."""
        refs = self._extras()
        if idx < 0 or idx >= len(refs):
            return
        refs[idx]["angle"] = float(angle)
        self._node.params["_patmax_model"] = self._model
        self._refresh_ref_list_item(idx)
        if hasattr(self, "_ref_list") and self._ref_list.currentRow() == idx:
            self._ref_field_updating = True
    ...
    def _train(self):
        if self._current_image is None:
            QMessageBox.warning(self, tr("Train"), tr("Chưa có ảnh. Hãy chạy pipeline trước."))
            return
        ...
        if self._node.tool.tool_id == "patmax_align":
            ...
            if not is_gray:
                QMessageBox.warning(
                    self, tr("PatMax Align — Train"),
                    tr("Input phải là ảnh GRAY.\n\n"
        ...
        if is_multi:
            ...
            if not shapes:
                QMessageBox.warning(self, tr("Train"),
                                    tr("Chưa có ROI nào. Vẽ ít nhất 1 vùng Pattern."))
                return
        else:
            if self._current_roi is None:
                QMessageBox.warning(self, tr("Train"),
                                    tr("Hãy kéo chuột trên ảnh để vẽ vùng Pattern trước."))
                return
            x, y, w, h = self._current_roi
            if w < 8 or h < 8:
                QMessageBox.warning(self, tr("Train"), tr("Vùng ROI quá nhỏ (min 8×8 px)."))
                return

        self._btn_train.setEnabled(False)
        self._train_status.setText(tr("Training..."))
        ...
        try:
            ...
            # Refresh extra-refs list + canvas markers (giữ qua retrain)
    ...
    def _run_search(self):
        if not self._model.is_valid():
            QMessageBox.warning(self, tr("Search"), tr("Chưa train model."))
            return
        ...
        if img is None:
            QMessageBox.warning(self, tr("Search"), tr("Không có ảnh để search."))
            return
        self._current_image = img
        self._image_sig = self._img_signature(img)
        ...
        try:
            ...
            # Build "objects" (mỗi result + extra refs đã transform) + apply
            # extra terminals — đồng nhất với proc_patmax khi pipeline chạy
            from core.tool_registry import (_build_patmax_objects,
                                              _apply_extra_terminals,
                                              _patmax_obj_origin_overrides)
            overrides = _patmax_obj_origin_overrides(node.params)
            objects = _build_patmax_objects(results, self._model, overrides)
            node.outputs["objects"] = objects
            if n > 0:
                best = results[0]
                # Top-level x/y = obj 0 đã resolve override (nếu user kéo)
                # → khớp với marker hiển thị.
                node.outputs["score"]    = best.score
                node.outputs["x"]        = objects[0]["x"]
                node.outputs["y"]        = objects[0]["y"]
                ...
                node.outputs["score"] = 0.0
                node.outputs["x"] = 0.0; node.outputs["y"] = 0.0
                node.outputs["angle"] = 0.0; node.outputs["scale"] = 1.0
            _apply_extra_terminals(node.outputs, objects, node.params)
    ...
    def _set_view(self, view: str):
        ...
        if view == "image":
            ...
            self._view_info.setText(tr("Output image with results"))
        elif view == "scoremap":
            self._img_label.set_origin(None, None)
            self._img_label.clear_extras()
            self._img_label.clear_obj_origins()
            ...
            if img is not None:
                self._img_label.set_image(img)
                self._view_info.setText(tr("Score heatmap (red=high match)"))
            else:
                self._img_label.set_image(None)
                self._view_info.setText(tr("Run search first"))
        elif view == "edges":
            self._img_label.set_origin(None, None)
            self._img_label.clear_extras()
            self._img_label.clear_obj_origins()
            if self._model.is_valid() and self._model.edge_image is not None:
                ...
                self._img_label.set_image(None)
                self._view_info.setText(tr("Train model first"))

    def _save_model(self):
        if not self._model.is_valid():
            QMessageBox.warning(self, tr("Save"), tr("Chưa có model để lưu."))
            return
        ...
        if path:
            try:
                ...
                # Nhớ path để fallback khi mở .aoi (model cũng được nhúng
                # trực tiếp trong .aoi qua to_dict).
                self._node.params["_patmax_model_file"] = path
                QMessageBox.information(self, tr("Save"), f"Model saved:\n{path}")
            except Exception as e:
    ...
    def _load_model(self):
        ...
        if path:
            ...
            if model and model.is_valid():
                ...
                # Refresh extra-refs list + canvas markers (load từ file)
                if hasattr(self, "_ref_list"):
                    self._refresh_references_list()
                self._refresh_canvas_extras()
                self._set_view("edges")
                QMessageBox.information(self, tr("Load"),
                    f"Model loaded OK\n"
                    f"Pattern: {model.pattern_w}×{model.pattern_h}  "
                    f"Edges: {model.edge_count}")
            else:
                QMessageBox.critical(self, tr("Error"), tr("Cannot load model or model invalid."))
    ...
    def _build_references_panel(self) -> QWidget:
        ...
        """
        grp = QGroupBox(tr("EXTRA REFERENCE POINTS  (Tham chiếu bổ sung)"))
        g = QVBoxLayout(grp)
        ...
        lbl_n = QLabel(tr("Name:")); lbl_n.setStyleSheet("color:#94a3b8; font-size:11px;")
        ...
        lbl_a = QLabel(tr("Angle:")); lbl_a.setStyleSheet("color:#94a3b8; font-size:11px;")
    ...
    def _refresh_canvas_extras(self):
        """Build extras list (image coords) từ model.extra_refs và gửi lên canvas.
        Tất cả refs hiển thị + kéo/xoay được đồng thời."""
        ...
        for j, ref in enumerate(refs):
            canvas_list.append({
                "name":  str(ref.get("name", f"Ref {j+1}")),
                "x":     rx0 + float(ref.get("x", 0.0)),
                "y":     ry0 + float(ref.get("y", 0.0)),
    ...
    def _on_extra_ref_selected(self, row: int):
        ...
        if row < 0 or row >= len(refs):
            self._set_extra_ref_fields_enabled(False)
            if hasattr(self, "_img_label"):
                self._img_label.set_extras_highlight(-1)
            return
        ref = refs[row]
        rx0, ry0 = self._roi_origin_xy()
        self._ref_field_updating = True
        ...
        # Highlight ref đang chọn trên canvas (vẫn kéo được tất cả)
        if hasattr(self, "_img_label"):
            self._img_label.set_extras_highlight(row)
        if hasattr(self, "_img_status"):
            name = self._ref_name.text() or f"Ref {row+1}"
            self._img_status.setText(
                f"✏ Đang chỉnh '{name}' — kéo marker bất kỳ trên ảnh "
                f"hoặc sửa Name/X/Y/Angle")
    ...
    def _on_add_reference(self):
        if self._model is None or not self._model.is_valid():
            QMessageBox.warning(
                ...
                tr("Train model trước khi thêm điểm tham chiếu."))
            return
        refs = self._extras()
        ...
        self._refresh_references_list()
        self._refresh_canvas_extras()
        # Auto-select ref vừa thêm → highlight trên canvas
    ...
    def _on_delete_reference(self):
        ...
        refs = self._extras()
        if row < 0 or row >= len(refs):
            QMessageBox.warning(self, tr("Delete"), tr("Chọn điểm tham chiếu trước."))
            return
    ...
    def _run_pipeline_node(self):
        ...
        img  = self._current_image if self._current_image is not None else self._get_input_image()
        if img is None:
            QMessageBox.warning(self, tr("Run"), tr("Không có ảnh."))
            return
        ...
        if self._model.is_valid():
            ...
            QMessageBox.warning(self, tr("Run"), tr("Train model trước."))
